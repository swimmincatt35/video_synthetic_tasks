...
from ..utils import linear, SiLU, conv_nd, ResBlock, Upsample, Downsample, normalization, zero_module, timeslice_broadcast, pairwise_frame_distances
...
class MultiAudioUNet(BaseUnet):
    def __init__(self, args, input_dim, h_dim):
        ...
        self.model_channels = args.multi_audio_model_channels
        self.num_res_blocks = args.multi_audio_num_res_blocks
        self.attention_resolutions = args.multi_audio_attention_resolutions
        self.dropout = args.multi_audio_dropout
        self.channel_mult = args.multi_audio_channel_mult
        self.conv_resample = args.multi_audio_conv_resample
        self.dims = 1  # Changed from 2 to 1 for audio
        self.num_heads = args.multi_audio_num_heads
        self.num_heads_upsample = args.multi_audio_num_heads_upsample
        self.use_scale_shift_norm = args.multi_audio_use_scale_shift_norm
    ...
    def forward(self, x, cond_emb, valid_mask, obs_mask, modality_stm_dict=None, ctx_vec=None):
        ...
        :param x: an [N x T x C_in x L] Tensor of latent inputs (L = sequence length).
        :param cond_emb: [N x D]
        :param valid_mask: [N x T]
        :param obs_mask: [N x T]

        :return: an [N x T x C_out x L] Tensor of outputs.
        """
        B, T, C, L = x.shape
        frame_dist = None
        ...
        for module in self.output_blocks:
            h = module(torch.cat([h, hs.pop()], dim=1), emb, T=T, attn_mask=1-valid_mask,
                    ...
                    gradient_checkpoint=gc)
        out = self.out(h)
        return out.reshape(B, T, C, L)

    @staticmethod
    def add_command_line_options(p):
        add_unet_cli(p, "multi_audio", 
                     channels=128, 
                     blocks=1, 
                     attn_res=[1, 2, 4], 
                     mult=[1, 2, 4], 
