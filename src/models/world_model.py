...
from models.inner_model import ActionConditionedWorldModelUnet
from models.encoders import RecurrentEncoder
from helpers.utils import summarize


__all__ = ["ActionConditionedWorldModel"]


class ActionConditionedWorldModel(AbstractDenoiser):
    def __init__(self,
                 args,
                 history_encoder: RecurrentEncoder,
                 inner_model: ActionConditionedWorldModelUnet):
    ...
    def forward(self,
        ...
        # Teacher forcing on observed steps
        ...
        # Choose c_noise for the noise embedding (use video if available, else first)
        if 'frame_latent' in edm_factors:
            c_noise = edm_factors['frame_latent'][3]
        else:
        ...
        rec_enc, pid_enc = self._get_obs_encoding(obs)  # [B, T, h_dim] each
        ...
        # Keep your odd-length padding for audio
        ...
        # State/action encoding uses the (teacher-forced) z, as before
        ...
        # Remove padding on audio_out
        if 'audio_out_latent' in model_out:
            model_out['audio_out_latent'] = model_out['audio_out_latent'][:, :, :-1, :]

        # Combine with per-modality c_skip / c_out
        out = {}
        for m in ("video", "audio_out"):
            latent_key = MODALITY_TO_LATENT[m]  # 'frame_latent' / 'audio_out_latent'
    ...
    def add_command_line_options(p):
        ActionConditionedWorldModelUnet.add_command_line_options(p)
    ...
    def create_model(args, continue_wandb_run):
        ...
        if continue_wandb_run:
            return ActionConditionedWorldModel.from_wandb(
                continue_wandb_run,
        ...
        inner_model = ActionConditionedWorldModelUnet(args)
        return ActionConditionedWorldModel(args, history_encoder, inner_model)
